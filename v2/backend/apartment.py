...
class ApartmentEncoder(json.JSONEncoder):
    def default(self, o) -> dict | str:
        if isinstance(o, Apartment):
            return {"number": o.number, "tenants": o.tenants, "visitors": o.visitors}
...
class ApartmentDatabase:
    ...
    def get_apt(self, number: int) -> Result[Apartment, ApartmentNotFound]:
        if number not in self.apt_to_row:
            return Failure(
                ApartmentNotFound(
                    f"Could not find apartment '{number}' in database of known apartments."
                )
            )
    ...
    def query(self, query, limit: int = -1) -> list[Tuple[Apartment, int]]:
        return process.extract(
            str(query), self.rows, limit=len(self.rows) if limit == -1 else limit
        )

    def add_tenant(
        self, name: str, number: int
    ) -> Result[None, DuplicateTenant | ApartmentNotFound]:
        match self.get_tenants(number):
            case Success(tenants):
                if name in tenants:
                    return Failure(
                        DuplicateTenant(
                            f"Could not add tenant '{name}' to apartment '{number}' because there is an exact duplicate tenant."
                        )
                    )
                tenants.append(name)
                self.save()
    ...
    def remove_tenant(
        self, name: str, number: int
    ) -> Result[None, TenantNotFound | ApartmentNotFound]:
        match self.get_tenants(number):
            case Success(tenants):
                if name not in tenants:
                    return Failure(
                        TenantNotFound(
                            f"Could not remove tenant '{name}' because he/she was not found in apartment '{number}'."
                        )
                    )
                tenants.remove(name)
                self.save()
    ...
    def add_visitor(
        self, name: str, number: int
    ) -> Result[None, DuplicateVisitor | ApartmentNotFound]:
        match self.get_visitors(number):
            case Success(visitors):
                if name in visitors:
                    return Failure(
                        DuplicateVisitor(
                            f"Could not add visitor '{name}' to apartment '{number}' because there is a visitor with the same name."
                        )
                    )
    ...
    def remove_visitor(
        self, name: str, number: int
    ) -> Result[None, VisitorNotFound | ApartmentNotFound]:
        match self.get_visitors(number):
            case Success(visitors):
                if name not in visitors:
                    return Failure(
                        VisitorNotFound(
                            f"Could not remove visitor '{name}' because he/she is not a visitor of apartment '{number}'."
                        )
                    )
